    ...
    foreach ($d in $dirs) {
        ...
        try {
            ...
            if ($upstream -like '*fatal*' -or [string]::IsNullOrWhiteSpace($upstream)) {
                ...
                if ($behind -gt 0 -and -not $dirty) {
                    Write-Host " [PULLING $behind COMMITS...]" -NoNewline -ForegroundColor Yellow
                    git pull --quiet 2>&1 | Out-Null
                    $aheadStr = (git rev-list --count '@{u}..HEAD' 2>&1).Trim()
                    $behindStr = (git rev-list --count 'HEAD..@{u}' 2>&1).Trim()
                    if ($aheadStr -match '^\d+$') { $ahead = [int]$aheadStr } else { $ahead = 0 }
                    if ($behindStr -match '^\d+$') { $behind = [int]$behindStr } else { $behind = 0 }
                }
